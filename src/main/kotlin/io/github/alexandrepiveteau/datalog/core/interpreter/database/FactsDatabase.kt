...
internal interface FactsDatabase : Iterable<PredicateWithArity> {
   ...
   * none is found.
   */
  operator fun get(predicate: PredicateWithArity): Relation
   ...
   */
  operator fun plus(other: FactsDatabase): FactsDatabase

  /** Returns a new [MutableFactsDatabase] with the same contents as this [FactsDatabase]. */
  fun toMutableFactDatabase(): MutableFactsDatabase
}

/** A mutable [FactsDatabase]. */
internal interface MutableFactsDatabase : FactsDatabase {
   ...
   */
  operator fun plusAssign(other: FactsDatabase)
   ...
   */
  operator fun set(predicate: PredicateWithArity, relation: Relation)
 ...
 * [PredicateWithArity].
 */
internal fun FactsDatabase.isEmpty(): Boolean = all { this[it].tuples.isEmpty() }

/**
 * Returns `true` if this [FactsDatabase] contains at least one fact for the given
 * [PredicateWithArity].
 */
internal fun FactsDatabase.isNotEmpty(): Boolean = !isEmpty()
